...
{
  /*
   * __hmax2 is available since CUDA 11.0
   */
...
void add_bias_relu_act(T* out, const T* bias, int m, int n)
{
  ...
  int ite = (n + blockDim.x - 1) / blockDim.x;
  int tid = threadIdx.x;
...
template <>
__global__
void add_bias_relu_act(half* out, const half* bias, int m, int n)
{
  half2 val, reg_bias;
  int row_id = blockIdx.x;
  ...
  for(int i = 0; i < ite; ++i)
  {
    ...
    while(row_id < m){
      ...
      out_ptr[tid + i * blockDim.x + row_id * n / 2] = relu<half2>(val);
      row_id += gridDim.x;
    }
  }
}
...
template <typename T>
__global__
void add_bias(half* out, const half* input, const half* bias, int m, int n)
{
  int tid = threadIdx.x;
  half2* out_ptr = (half2*)out;
